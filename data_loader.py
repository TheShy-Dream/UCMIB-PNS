    ...
    def collate_fn(batch):
        ...
        if not hp.dataset == "sims":
            for sample in batch:
                ...
                ids.append(sample[2])
                label_T,label_A,label_V=None,None,None
        else:
            for sample in batch:
                if len(sample[0]) > 4:  # unaligned case
                    v_lens.append(torch.IntTensor([sample[0][4]]))
        ...
        return sentences, visual, vlens, acoustic, alens, labels, lengths, bert_sentences, bert_sentence_types, bert_sentence_att_mask, ids,v_masks,a_masks,label_T,label_A,label_V
