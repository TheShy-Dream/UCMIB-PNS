...
class InfoNCE(nn.Module):
    ...
    def __init__(self,modal1_dim_in,modal2_dim_in, embed_dropout,temperature=0.1, reduction='mean', negative_mode='unpaired',infonce_type="maximum"):
        super().__init__()
    ...
    def forward(self, query, positive_key, negative_keys=None):
        ...
        return info_nce(proj_query, proj_positive, negative_keys,
                        ...
                        infonce_type=self.infonce_type
                        )


def info_nce(query, positive_key, negative_keys=None, temperature=0.1, reduction='mean', negative_mode='unpaired',infonce_type="maximum"):
    ...
    if negative_keys is not None:
        ...
        if infonce_type=="maximum":
            # Cosine between all combinations
            logits = query @ transpose(positive_key)

            # Positive keys are the entries on the diagonal
            labels = torch.arange(len(query), device=query.device)
        elif infonce_type=="minimum":
            # Cosine between all combinations
            logits = query @ transpose(positive_key) /temperature
            logits = F.log_softmax(logits,dim=1)
            # 创建一个K * K的全零矩阵
            labels = torch.zeros(len(query), len(query),device=query.device,dtype=torch.float32)
            # 将除对角线外的元素设置为1/(K-1)
            labels.fill_(1 / (len(query) - 1))
            mask = torch.eye(len(query), dtype=torch.bool)
            labels[mask] = 0
            return F.kl_div(logits, labels, reduction=reduction),logits
            # Positive keys are the entries on the diagonal
        elif infonce_type=="uniform":
            # Cosine between all combinations
            logits = query @ transpose(positive_key) /temperature
            logits = F.log_softmax(logits,dim=1)
            # Positive keys are the entries on the diagonal
            labels = torch.ones(len(query), len(query),device=query.device,dtype=torch.long) / len(query)
            return F.kl_div(logits,labels,reduction=reduction),logits
