...
class LanguageEmbeddingLayer(nn.Module):
    ...
    def __init__(self, hp):
        ...
        if "sims" == hp.dataset:
            bertconfig = BertConfig.from_pretrained("/home/cjl/code/MMIM/TextBackbone/bert-base-chinese/", output_hidden_states=True)
            self.bertmodel = BertModel.from_pretrained("/home/cjl/code/MMIM/TextBackbone/bert-base-chinese/", config=bertconfig)
        else:
            bertconfig= BertConfig.from_pretrained("/home/cjl/code/MMIM/TextBackbone/bert-base-uncased/",output_hidden_states=True)
            self.bertmodel =BertModel.from_pretrained("/home/cjl/code/MMIM/TextBackbone/bert-base-uncased/", config=bertconfig)

    def forward(self, sentences, bert_sent, bert_sent_type, bert_sent_mask):
        if not bert_sent_type.size(1)==1:
...
class CrossAttention(nn.Module):
    ...
    def forward(self, input_modal1, input_modal2, Tmask=None, Amask=None, Vmask=None, whether_add_position=True,
                compensation=None, compensation_modal=None):  # 这里下面都不用给counterfactual attention type
        """
        传入格式：(seq_len, batch_size,emb_size)
        t: torch.Size([50, 32, 768])
        a: torch.Size([134, 32, 5])
        ...
        if self.hp.d_tin==self.d_modal1 and self.hp.d_ain==self.d_modal2:
            encoded= self.net(modal1, modal2, modal2,Tmask,Amask,whether_add_position=whether_add_position)
        elif self.hp.d_tin==self.d_modal1 and self.hp.d_vin==self.d_modal2:
            encoded = self.net(modal1, modal2, modal2, Tmask, Vmask,whether_add_position=whether_add_position)
        elif self.hp.d_ain==self.d_modal1 and self.hp.d_tin==self.d_modal2:
            encoded = self.net(modal1, modal2, modal2, Amask, Tmask, whether_add_position=whether_add_position)
        elif self.hp.d_ain==self.d_modal1 and self.hp.d_vin==self.d_modal2:
            encoded = self.net(modal1, modal2, modal2, Amask, Vmask, whether_add_position=whether_add_position)
        elif self.hp.d_vin==self.d_modal1 and self.hp.d_tin==self.d_modal2:
            encoded = self.net(modal1, modal2, modal2, Vmask, Tmask, whether_add_position=whether_add_position)
        elif self.hp.d_vin == self.d_modal1 and self.hp.d_ain == self.d_modal2:
            encoded = self.net(modal1, modal2, modal2, Vmask, Amask, whether_add_position=whether_add_position)
        ...
        #专门给因果注意力用的
            ...
            if compensation_modal=="text" and self.hp.d_ain==self.d_modal2:
                encoded= self.net(compensation, modal2, modal2,None,Amask,whether_add_position=whether_add_position)
            elif compensation_modal=="text" and self.hp.d_vin==self.d_modal2:
                encoded = self.net(compensation, modal2, modal2, None, Vmask, whether_add_position=whether_add_position)
            elif compensation_modal=="audio":
                encoded = self.net(modal1,compensation,compensation,Tmask,None,whether_add_position=whether_add_position)
            elif compensation_modal=="vision":
                encoded = self.net(modal1,compensation,compensation,Tmask,None,whether_add_position=whether_add_position)
...
class SelfAttention(nn.Module):
    ...
    def forward(self,input,maskT=None,maskA=None,maskV=None,whether_add_position=True):
        ...
        output=self.output_linear(self.linear(encoded))#放到原来的
        return output

class SelfAttentionAudio(nn.Module):
    def __init__(self,hp, d_in,d_model, nhead, dim_feedforward, dropout,num_layers=6):
        super(SelfAttentionAudio, self).__init__()
        self.hp=hp
        self.d_in=d_in
        self.num_heads = nhead
        self.d_model=d_model
...
class SelfAttentionVision(nn.Module):
    def __init__(self,hp, d_in,d_model, nhead, dim_feedforward, dropout,num_layers=6):
        super(SelfAttentionVision, self).__init__()
        self.hp=hp
        self.d_in=d_in
        self.num_heads = nhead
        self.d_model=d_model
...
class CLUB(nn.Module):
    ...
    def forward(self, modal_a, modal_b, sample=False): #这里本来算的是两个模态的
        """
            ...
            Args:
                ...
                modal_b (Tensor): y in above equation
        """
        ...
        # 边缘概率的期望往往意味着要复制张量，计算求和
        positive = - (mu - modal_b) ** 2 / 2. / torch.exp(logvar)#E_p(x,y)[log q(y|x)]
        negative = - torch.mean((true_tile - pred_tile) ** 2, dim=1) / 2. / torch.exp(logvar)#E_p(x)p(y)[log q(y|x)]
