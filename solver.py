...
class Solver(object):
    ...
    def train_and_eval(self):
        ...
        def train(model, optimizer, criterion, stage=1, current_epoch=0):
            epoch_loss = 0
            ...
            for i_batch, batch_data in enumerate(self.train_loader):
                text, visual, vlens, audio, alens, y, l, bert_sent, bert_sent_type, bert_sent_mask, ids,v_mask,a_mask,label_T,label_A,label_V = batch_data
                ...
                with torch.cuda.device(0):
                    ...
                    if label_T is not None and label_A is not None and label_V is not None:
                ...
                lld, nce, preds, pn_dic, H,infonce_loss,head_loss,kl_loss,pred_final,text_pred,visual_pred,audio_pred = model(text, visual, audio, vlens, alens,
                ...
                if stage == 1:  # TASK+BA+CPC
                    ...
                    if self.hp.dataset =="sims":
                        a_loss = criterion(audio_pred, label_A)
                        t_loss = criterion(text_pred, label_T)
                        v_loss = criterion(visual_pred, label_V)
                    else:
                        a_loss = torch.tensor(0.0,dtype=torch.float32,device=torch.device("cuda:0"))
                        v_loss = torch.tensor(0.0,dtype=torch.float32,device=torch.device("cuda:0"))
                        t_loss = torch.tensor(0.0,dtype=torch.float32,device=torch.device("cuda:0"))
                    # update memory
                    ...
                    if self.hp.contrast:
                        loss = y_loss + self.alpha * nce - self.beta * lld + self.eta1*infonce_loss+ self.eta2*head_loss + self.eta3 * kl_loss + self.eta4 * pred_final + self.eta5 * (a_loss+v_loss+t_loss)
                    else:
                        loss = y_loss
                    if i_batch > mem_size:
                        loss -= self.beta * H
        ...
        def evaluate(model, criterion, test=False):
            ...
            with torch.no_grad():
                for batch in loader:
                    text, vision, vlens, audio, alens, y, lengths, bert_sent, bert_sent_type, bert_sent_mask, ids,v_mask,a_mask,label_T,label_A,label_V = batch
                    ...
                    _, _, preds, _, _,_,_,_,_,_,_,_= model(text, vision, audio, vlens, alens, bert_sent, bert_sent_type,
        ...
        for epoch in range(1, self.hp.num_epochs + 1):
            ...
            train_loss = train(model, optimizer_main, criterion, 1,current_epoch=epoch)
